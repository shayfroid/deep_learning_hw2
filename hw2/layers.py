...
class LinearVariance2(ModuleWrapper):
...
class LinearVarianceBe(ModuleWrapper):
    ...
    def forward(self, x):
        if self.training:
            ...
            eps = torch.bernoulli(self.probs) - 0.5
...
class LinearVarianceDiscrete(ModuleWrapper):
    ...
    def __init__(self, in_features, out_features, bias=True):
        super(LinearVarianceDiscrete, self).__init__()
        self.in_features = in_features
        self.out_features = out_features
        self.W = Parameter(torch.Tensor(out_features, in_features))
        if bias:
    ...
    def forward(self, x):
        if self.training:
            eps = Variable(torch.round((self.W.data.new(self.W.size()).uniform_()- 0.5)*255)/255 )
        else:
            eps = 0.0
        output = F.linear(x, self.W*eps)
        if self.bias is not None:
